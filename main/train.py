...
def train(args):
    ...
    # データセットとデータローダーの準備
    if (args.dataset == "audiocaps"  or  args.dataset == "fsd50k"  or  args.dataset == "clotho"  or  args.dataset == "macs"):
        ...
        dataset_names = ["audiocaps", "fsd50k", "clotho"]
        train_dataset = ConcatDataset([
            FeaDataset(dataset=name, split="train", hidden_dim=args.hidden_dim)
            for name in dataset_names
        ])
        val_dataset = ConcatDataset([
            FeaDataset(dataset=name, split="val", hidden_dim=args.hidden_dim)
            for name in dataset_names
        ])
