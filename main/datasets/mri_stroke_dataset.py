...
class MriStrokeDataset(Dataset):
    def __init__(self, text_tokenizer, audio_processor, split: str, root: str = "../data"):
        ...
        self.samples = self.get_audio_path()


    # add prompt
    def get_class(self):
        classes = []
        categories = ["bheem", "cha", "dheem", "dhin", "num", "ta", "tha", "tham", "thi", "thom"]
    ...
    def get_audio_path(self):
        samples = []
        root_audio_folder = self.root + "/mri_stroke"
        for folder in ["B", "C", "C#", "D", "D#", "E"]:
            for filename in os.listdir(f"{root_audio_folder}/{folder}"):
                if (not filename.endswith(".wav")):
                    continue
                class_ = filename.split("__")[2].split("-")[0]
                class_ = f"{self.prompt} {class_}"
                samples.append((f"{root_audio_folder}/{folder}/{filename}", class_))
        return samples
