...
class TUT2017Dataset(Dataset):
    ...
    def get_class(self):
        classes = []
        evautaiton_file = self.root + "/TUT2017/TUT-acoustic-scenes-2017-evaluation/evaluation_setup/evaluate.txt"
        for line in open(evautaiton_file, "r"):
            _, class_ = line.split("\t", 1)
            class_ = f"{self.prompt} {class_}"
            if(class_ not in classes):
                classes.append(class_)
        # print(len(classes), "classes in TUT2017 dataset.")
        return classes
    ...
    def get_audio_path(self):
        ...
        evautaiton_file = self.root + "/TUT2017/TUT-acoustic-scenes-2017-evaluation/evaluation_setup/evaluate.txt"
        for line in open(evautaiton_file, "r"):
            audio_path, class_ = line.split("\t", 1)
            if (not audio_path.endswith(".wav")):
                continue
            class_ = f"{self.prompt} {class_}"
            samples.append((f"{self.root}/TUT2017/{audio_path.strip()}", class_))
        return samples
